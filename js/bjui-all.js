...
+function ($) {
    ...
    var BJUI = {
        ...
        alertMsg: {displayPosition:'topcenter', alertTimeout: 1500}, //alertmsg display position && close timeout
        ajaxTimeout: 30000,
        ...
        initLayout: function() {
            ...
            if (ww < BJUI.ui.showSideWidth) {
                $sidenavcol.css('left', -(BJUI.ui.sidenavWidth )).addClass('autohide')
...
+function ($) {
    ...
    $(document).on('click.bjui.sidenav.data-api', '[data-toggle="sidenav"]', function(e) {
        ...
        var $body = $('body'), $this = $(this), href = $this.data('url') || $this.attr('href'),
            cache = $this.data('cache'),
            $li   = $(this).parent(), $box = $('#bjui-sidenav-box'),
            $panels = $li.data('bjui.sidenav.hnav.panels')
...
+function ($) {
    ...
    Navtab.prototype.openTab = function() {
        ...
        if (iOpenIndex >= 0) {
            ...
            var tabFrag = '<li><a href="javascript:" title="#title#"><span>#title#</span></a><span class="close">×</span></li>',
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        $(e.target).find('[data-toggle="autoajaxload"]').each(function() {
            var $element = $(this), data = $element.data(), options = data.options
...
+function ($) {
    ...
    Pagination.prototype.init = function() {
        ...
        if(options.width>0) that.$element.find('> .paging-content').width(options.width).initui()
                        ...
                        if (n === 'add') {
                            ...
                            that.$toolbar_add = $(btnHtml).attr('data-icon', 'sign-out').addClass('btn-green').text(options.exportName || BJUI.getRegional('datagrid.export'))
                                ...
                                .on('click', function(e) {
                                    if (options.exportOption) {
                                        ...
                                        if (opts.options && opts.options.url) {
                                            opts.options.data = {}
                                            //that.$element.data('filterDatas') || {},
                                            $.extend(opts.options.data, that.sortData || {})
                            ...
                            that.$toolbar_add = $(btnHtml).attr('data-icon', 'filter').addClass('btn-green').text(options.exportfName || BJUI.getRegional('datagrid.exportf'))
                                ...
                                .on('click', function(e) {
                                    if (options.exportOption) {
                                        var opts = options.exportOption,
                                            filterDatas = that.tools.getRemoteFilterData(true)
                                        ...
                                        if (opts.options && opts.options.url) {
                                            ...
                                            if (options.postData) {
                                                ...
                                                if (typeof options.postData === 'object') {
                                                    $.extend(opts.options.data, options.postData);
                                                }
                                            }
                                            opts.options.type = 'POST'
    ...
    Tags.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            createTag: function(label, value) {
                ...
                var $btn = $('<span class="label label-tag" data-value="' + value +'" style="margin-left: 1px; margin-top: 1px;"><i class="fa fa-tag"></i> ' + label + '  <a href="#" class="close">×</a></span>')
            ...
            $selectzTree.each(function() {
                ...
                $this.click(function() {
                    ...
                    if(!$this.attr("readonly")){
                        //setting.async.oldurl = setting.async.url;keydown
                        $this.on("keyup", function(event) {
                            var q = this.value;
                            //console.log(q);
                            if((q && q.trim().length>1) || (this.oldvalue && q!=this.oldvalue)){
                                var zTree    = $.fn.zTree.getZTreeObj(treeid);
                                zTree.setting.async.otherParam.q = q;
                            ...
                            var navinput = $.CurrentNavtab.find('#'+$this.attr("id")+"_value");
                            var Diainput = null;
                            //instanceof
                            if(typeof $.CurrentDialog!='undefined') Diainput = $.CurrentDialog.find('#'+$this.attr("id")+"_value");
                            var valueid = null;
                            if(navinput && navinput.length)
                            {
                                valueid = navinput.val();
                            }
                            else if(Diainput && Diainput.length)
                            {
                                valueid = Diainput.val();
                            }
            ...
            var initWebUploader = function($element, index) {
                ...
                if (options) {
                    ...
                    if (typeof options === 'object') {
                        ...
                        var $wrap = $('<div id="uploader" class="wu-example"><div class="queueList"><div id="dndArea" class="placeholder"><div id="filePicker" style="width: 156px;margin: auto"></div><p>或将文件拖到这里</p></div></div><div class="statusBar" style="display:none;"><div class="progress"><span class="text">0%</span><span class="percentage"></span></div><div class="info"></div><div class="btns"><div id="filePicker2"></div></div></div>'),
                        // 图片容器
